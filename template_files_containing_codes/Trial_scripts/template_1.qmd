...
data_dict_temp = reshape(data_dict["t2m"], (66,27,271752))
...
heatmap(temp_long,temp_lat,daily_temp_data[:,:,1],xlabel="Longitude",ylabel="Latitude",title="Temperature on $(time_entries_all[1])")
...
heatmap(temp_long,temp_lat,data_dict[:,:,1],xlabel="Longitude",ylabel="Latitude")

```


# finding daily temp from hourly data from ERA5 Air temp data
```{julia}
display(time_entries_all[end])
```

# Extracting Houston's lon lat temp data
```{julia}
